...
<details>
<summary>⚠️ Troubleshooting - TODO items not loading</summary>

### Items stuck on loading spinner?

The Cosmos DB account may have been created with local authentication disabled. The Python API connects with a connection string and needs local auth enabled.

> 💡 Replace `<env-name>` with the name you chose during `azd init`.

**Step 1** — Set your resource group name

```powershell
$RG = "rg-<env-name>"
```

**Step 2** — Get the Cosmos DB and API app names
...
```powershell
$API = az webapp list --resource-group $RG --query "[?contains(name,'api')].name" -o tsv
```

**Step 3** — Enable Cosmos DB local authentication

```powershell
az resource update --ids $(az cosmosdb show -n $COSMOS -g $RG --query id -o tsv) --set properties.disableLocalAuth=false
```

**Step 4** — Restart the API

```powershell
az webapp restart -n $API -g $RG
```

Refresh the browser — the TODO items should now load.
